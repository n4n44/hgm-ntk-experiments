...
def remove_covar(dat,small_covar,large_covar):
    ...
    for i in range(n):
        ...
        if ((mydet > small_covar) and (mydet < large_covar)) :
            newdat.append(list(dat[i]))
        else:
            removed.append(list(dat[i]))
    return [np.array(newdat),removed]
...
def sort_dat(dat,small_covar=1e-3,large_covar=np.inf,threshould=1e-5,initial_point=np.array([[-1,0,-1, -1]]),even=True):
    ...
    if (type(even)==type(True)):
        ...
        dat1=dat
    [dat2,removed_small_covar]=remove_covar(dat1,small_covar,large_covar)
    #print('dat=',dat)
    #print('dat2=',dat2)
    ...
    return [dat5,np.array(removed),np.array(removed_small_covar)]
